...
function toFormValues(profile: any) {
  if (!profile) return {};
  return { ...profile, care_types_input: profile.care_types ?? [] };
}

function fromFormValues(data: any) {
  const { care_types_input, ...rest } = data;
  if (care_types_input !== undefined) rest.care_types = care_types_input;
  return rest;
}
...
const ElderForm = ({ data, register, control, setValue, watch, disabled }: any) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <div className="sm:col-span-2"><Field label="Condições médicas">
...
const CaregiverForm = ({ register, control, watch, setValue, disabled }: any) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div className="sm:col-span-2"><Field label="Bio">
        <textarea rows={3} {...register("bio")} disabled={disabled} placeholder="Fale um pouco sobre você e sua experiência..." className={textareaClass} />
      </Field></div>
...
const ProfessionalForm = ({ register, control, watch, setValue, disabled }: any) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <div className="sm:col-span-2"><Field label="Bio / Apresentação">
      <textarea rows={3} {...register("bio")} disabled={disabled} placeholder="Descreva sua especialidade e experiência..." className={textareaClass} />
    </Field></div>
  </div>
);

const InstitutionForm = ({ register, control, disabled }: any) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <Field label="CNPJ">
      <input {...register("cnpj")} disabled={disabled} placeholder="00.000.000/0000-00" className={inputClass} />
    </Field>
  ...
  ({ role, isEditing }, ref) => {
    ...
    return (
      <form className="space-y-6">
        ...
        {role === "INSTITUTION"  && <InstitutionForm  {...formProps} />}
      </form>
    );
  }
);
